...
from bottle import template, static_file, request, redirect, response
...
def checkinDB(value_to_check,table_var, column_var, id_name_in_db='"%"', id='%'):
    ...
    command='SELECT IIF ("{string}" IN (SELECT {column} FROM {table} WHERE {id_name_in_db}="{id}") , "MATCH", "MISS")'.format(string=value_to_check, table=table_var, column=column_var, id=id, id_name_in_db=id_name_in_db)
...
#main body of the function. It lookes for username, random string saved at the login and logged in status in database
    if checkinDB(loginName, "users", "username") == "MATCH" and checkinDB(randStr, "users", "randStr", "userID", id) == "MATCH" and checkinDB("1","users","loggedIn")=="MATCH":
...
def updateinDB(item_ID, table, column, value, ID_field_name_in_Database):
    ...
    command='UPDATE {table} SET {column}={value} WHERE {DB_ID}="{item_ID}"'.format(table=table,column=column,value=value,item_ID=item_ID, DB_ID=ID_field_name_in_Database)
    c.execute(command)
    con.commit()
...
def getFromDB(table_var, column_var, id_var, checked_userID):
    command='SELECT {column} FROM {table} WHERE {id_var}="{checked_userID}"'.format(column=column_var, table=table_var,id_var=id_var, checked_userID=checked_userID)
    c.execute(command)
    result=c.fetchone()
    if result!=None:
        return result[0]
...
def create_address_package(formdata, userID):
    ...
    mapping_dictionary={'streetname':'street', 'streetnumber':'building', 'flatnumber':'flat', 'city':'city', 'zipcode':'zipcode'}
...
def insert_into_db(table, dictionary, commit):
    ...
    command='INSERT INTO {table} ({keylist}) VALUES ({vallist})'.format(table=table, vallist=vallist_round, keylist=keylist_round)
    c.execute(command)
    if commit==1:
        con.commit()
...
#4.1.0 Check how newcustomer page was entered
    ...
    if from_page=="1":
        ...
        return template('newcustomer', duplicate_password_flag_HTML=False, password_dont_match_HTML=False, duplicate_account_flag_HTML=False,loginINFO=login_status)
    ...
    if fromdone == "1":
        ...
        if password1 != password2:
            ...
            return template('newcustomer', password_dont_match_HTML=password_dont_match, duplicate_account_flag_HTML=False, loginINFO=login_status)
        else:
            ...
            if duplicate == "MATCH":
                ...
                return template('newcustomer', password_dont_match_HTML=False, duplicate_account_flag_HTML=duplicate_account_flag, loginINFO=login_status)
            else:
